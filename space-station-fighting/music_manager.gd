...
func _fade_to(target_db: float, time_sec: float, stop_on_end := false):
    ...
    if get_tree().has_method("create_tween"):
        # Godot 4 path
        if _tween:
            # try to kill previous tween if possible
            if _tween is SceneTreeTween:
                _tween.kill()
        _tween = get_tree().create_tween()
        _tween.tween_property(player, "volume_db", target_db, time_sec)
        if stop_on_end:
            _tween.connect("finished", self, "_on_fade_out_finished")
    else:
        ...
        tw.start()
        if stop_on_end:
            tw.connect("tween_all_completed", self, "_on_fade_out_finished")
